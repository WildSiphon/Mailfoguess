...
class Guesser:
    ...
    def __init__(self, user: User, level="min", separators="-._"):
        ...
        # Useful generators
        self._localpart_generator = LocalPartGenerator(
            level=level, separators=separators
        )
    ...
    def validate(self, validate_all=False):
        ...
        :param validate_all: flag to validate everything without asking the user
        ...
        for provider in self._emails:
            ...
            if provider.split(".")[0] not in self._email_generator.validators:
                ...
                if validate_all:
                    ...
                    answer = input(
                        f"Would you like to verify {provider} ({len(self._emails[provider])})\t[N/y] "
                    )
                ...
                if answer.lower() in ["y", "yes"]:
                    ...
                    for email in self._emails[provider]:
                        if re.fullmatch(self.REGEX, email):
                            ...
                            if flag:
                                flag = self._validated_emails[provider][email] in [
                                    -1,
                                    None,
                                    "null",
                                ]
                                ...
                                while self._validated_emails[provider][email] == -1:
                                    try:
                                        self._validated_emails[provider][
                                            ...
                                            self._email_generator.validate_email, email
                                        )
                    ...
                    for email in self._emails[provider]:
                        if email not in self._validated_emails[provider]:
                            ...
                            pass
                    print(
                        '> All emails already verified remain unchanged, the others are set "unverified"'
                    )
    ...
    def verified_emails(self, provider):
        ...
        return [
            email
            for email in self._validated_emails[provider]
            if self._validated_emails[provider][email] is True
        ]
